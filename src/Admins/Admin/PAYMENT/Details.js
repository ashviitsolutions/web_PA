...
function Details() {
    ...
    useEffect(() => {
        ...
        selectedCheckboxes.forEach(checkboxId => {
            ...
            if (selectedService) {
                updatedTotalAmountTax += selectedService.amount_calculation?.taxAmount || 0;
                updatedGratuatity += selectedService.amount_calculation?.tipAmount || 0;
            }
        });
                            ...
                            {apidata && apidata.services.map((service, index) => (
                                <tr key={index}>
                                    <td>
                                        <input type="checkbox"
                                            onChange={(event) => handleCheckboxChange(
                                                event, service._id,
                                                service?.provider_amount_calculation?.servicePrice,
                                                service?.provider_amount_calculation?.addonAmount,
                                                service?.provider_amount_calculation?.tipAmount,
                                                service?.amount_calculation?.totalAmountWithTax,
                                                service?.amount_calculation?.taxAmount,
                                    ...
                                    <td className="">
                                        ...
                                        <p className='sub title'><span>Charges: {service?.amount_calculation?.serviceAmount}$</span></p>
                                        <p className='sub title'><span>Comm. {service?.provider_amount_calculation?.servicePrice}$</span></p>
                                    </td>
                                    <td className="sub">
                                        ...
                                        <p className='title'><span>Charges: {service?.amount_calculation?.addonAmount}$</span></p>
                                        <p className='title'><span>Comm. {service?.provider_amount_calculation?.addonAmount?.toFixed(2)}$</span></p>
                                    </td>
                                    <td>{service?.provider_amount_calculation?.tipAmount?.toFixed(2)}$</td>
                                    <td>{service?.amount_calculation?.totalAmountWithTax?.toFixed(2)}$</td>
